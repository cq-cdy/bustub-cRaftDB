...
#include <grpc++/grpc++.h>
#include "raft/craft/peers.h"
#include "raft/craft/utils/commonUtil.h"
...
public:
    ...
    CRaftClient(std::vector<std::string> peersAddr) {
         for(auto addr:peersAddr){
            if(!isValidIpPort(addr)){
                spdlog::error("addr error:{}",addr);
                exit(1);
            }
        }
        m_peersAddr  = std::move(peersAddr);
    }
