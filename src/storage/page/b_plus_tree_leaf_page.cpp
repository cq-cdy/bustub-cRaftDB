...
auto B_PLUS_TREE_LEAF_PAGE_TYPE::KeyIndex(const KeyType &key, const KeyComparator &keyComparator) const -> int {
  auto target = std::lower_bound(array_, array_ + GetSize(), key, [&keyComparator](const auto &pair, auto k) {
    return keyComparator(pair.first, k) < 0;  // < 0 表示第一个元素小于第二个元素
  });
...
auto B_PLUS_TREE_LEAF_PAGE_TYPE::Insert(const KeyType &key, const ValueType &value, const KeyComparator &keyComparator)
    -> int {
  auto distance_in_array = KeyIndex(key, keyComparator);  // 找到key插入的位置
  if (distance_in_array == GetSize()) {                   // 如果待插入的位置在末尾
    *(array_ + distance_in_array) = {key, value};
    IncreaseSize(1);
    return GetSize();
  }

  if (keyComparator(array_[distance_in_array].first, key) == 0) {  // 如果该位置已经存在 相同的key
    return GetSize();
  }
  //注意source的范围是左闭右开，move_backward是从后向前复制，即先把 array_[GetSize() - 1] 移动到array_ [GetSize()]处
  //再把array_ [GetSize() - 2] 移动到array_ [GetSize() - 1]处
  //最后把array_[distance_in_array] 移动到 array_ [distance_in_array + 1]处
  // 即把从 distance_in_array 开始的后面整体元素，像后移动一位
  std::move_backward(array_ + distance_in_array, array_ + GetSize(), array_ + GetSize() + 1);
  *(array_ + distance_in_array) = {key, value};  // 第 array_ + distance_in_array 个位置为新插入的 kv
  IncreaseSize(1);
  return GetSize();
}
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::CopyNFrom(MappingType *items, int size) {
  std::copy(items, items + size, array_ + GetSize());  // 复制添加到当前array_的末尾
  IncreaseSize(size);
}
